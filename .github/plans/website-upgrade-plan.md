...
- **Goal:** Modernize the legacy jQuery site to a full-stack, scalable, and maintainable platform.
- **Frontend:** Next.js (SSG), React, Tailwind CSS, shadcn/ui, hosted on GitHub Pages (free).
- **Backend:** Node.js (Express/Fastify), Prisma, Supabase PostgreSQL, hosted on Render/Fly.io (free).
- **Key Features:** Booking system, member authentication (NextAuth.js), admin dashboard, analytics, payments (Stripe/Square).
- **Domain:** www.renofitness.ca via GoDaddy, with DNS configured for GitHub Pages.
- **Zero fixed costs:** All core services run on free tiers; only pay per payment transaction.
- **Phased implementation:** Foundation, core pages, auth, booking, analytics, performance, testing, and launch.
- **Risk mitigation:** Free tier limits, data migration, and security are addressed.
- **DNS setup instructions** included for custom domain.
...
| Payments  | Stripe or Square    | Payment processing            |
...
- ✅ Visitors can submit questions/comments via an online form without sign up/sign in, and submissions are delivered directly to the owner.
...
- [ ] Add online form for questions/comments (no sign up required) to allow direct visitor interaction
...
- [ ] Deploy backend to Render or Fly.io (free tier)
...
- Domain registrar: GoDaddy (www.renofitness.ca)
- To use with GitHub Pages:
  1. In your GoDaddy DNS settings, add or edit the following records:
...
| Payment Processing |  _Variable_  | Stripe: 2.9% + $0.30 or Square: 2.6% + $0.10 per transaction                    |
| Domain             | **$0-12/yr** | GoDaddy (www.renofitness.ca) as registrar; custom domain points to GitHub Pages |
| **TOTAL**          |    **$0**    | Only pay per Stripe/Square transaction                                          |
